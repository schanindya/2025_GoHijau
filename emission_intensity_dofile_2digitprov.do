...
import dbase "/Users/schalkeanindya/Downloads/803/ibs21_kbli_2_diseminasi_digit2.dbf", clear

* Merge RGS datasets
merge 1:1 RENUM2 DISIC2 using `ibs1', keep(3) nogen
...
foreach type in `energy_types' {
    local suffix = substr("`type'", 1, 3)
    gen `suffix'VCU21_PER_ENERGY = `suffix'VCU21 / TJ_`type'
}
...
replace VTLVCU = VTLVCU * 1000

* 6.2 Calculate emission intensity (CO2 per million Rp value added)
gen intensity = (TOTAL_CO2 / VTLVCU) * 1000000

* 6.3 Collapse data by province and industry
collapse (median) intensity (sum) TOTAL_CO2 TOTAL_KWH TOTAL_TJ, by(DPROVI21 DISIC2)

* Add variable labels
label variable TOTAL_TJ "Total energy content (TJ)"
label variable TOTAL_KWH "Total energy content (kWh)"
label variable intensity "CO2 emissions per Rp1 million value added"

save "$workdir/CO2_EMISSION_Prov_2digit", replace

* Merge with KBLI data
import excel "/Users/schalkeanindya/Downloads/KBLI2020_2 digit.xlsx", sheet("Sheet1") firstrow clear
rename DISIC521 DISIC2
tostring DISIC2, replace
merge 1:m DISIC2 using "$workdir/CO2_EMISSION_Prov_2digit", nogen

* Create province code
gen provcode = DPROVI21 + "00"
